...
`/agent` 页面采用「顶部控制栏 → 左栏会话 → 中部消息列」三区布局，底部为输入区与遥测条。

**顶部控制栏**（单行，52px）：智能体选择、**模型**、**默认工作流**、查看运行轨迹、工具与权限、智能体设置、刷新。模型与默认工作流会**直接写回当前智能体配置**，不是本次请求的临时参数——Agent 的 run 载荷不含 model/mode 字段，而这两项决定运行时实际行为，此前只能进设置弹窗修改，因此放在顶栏并给出「已保存到智能体配置」反馈。窄屏下这两个下拉与次要按钮按 `.agent-rail-optional` 类名折叠（不要用 `:nth-child` 序号，新增控件会静默错位）。

其下是一行**待确认横幅**：仅在有待确认操作时出现，显示准确条数；点「定位到确认卡片」会切回对话页签并滚动到确认卡片。可操作的确认卡片仍在消息列顶部，两者不重复。

**左栏会话**：最近会话 + 活跃智能体摘要。会话项带状态点，但**状态点只对当前会话能给全四种状态**（运行中 / 待确认 / 完成 / 失败）——run 级状态在 `AgentRun` 上，而 `/agent/threads` 列表只返回 `thread.status`，其取值域实际仅 `active` / `archived`（归档已被列表过滤）。因此按「有数据才显示」实现，不显示推测状态。要让全部会话都有准确状态点，需后端在列表接口补「最近一次 run 的状态」。

**消息列**：宽度受限并居中，气泡按角色区分（用户右侧 / 助手左侧）且带时间戳；工具调用与步骤轨迹**默认折叠**（仅运行中展开），失败与待确认在折叠状态下仍可见（summary 计数 Tag + 常显确认卡片）。

**底部遥测条**：Run 状态、步骤数、工具数、耗时、Token、成本，等宽字体、缺失显示 `--`。**后端缺口**：`AgentRun` 目前既无 `duration_ms` 也无 `total_tokens`/`cost`，`AIUsageLog` 虽有 token 与成本但未与 run 关联，故 Token 与成本当前恒为 `--`；前端已声明可选字段，后端补齐后自动生效。

**视觉约定**：优先用分组分隔线与留白；只有需要表达层级时才用边框卡片（待确认、错误隔离、选中态、Markdown 表格）。
