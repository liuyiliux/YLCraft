...
| Agent 声明式团队组合 | 运行时骨架已落地：`AgentScope` 主机/代理平面隔离、团队模板 schema/loader/validator、`TeamComposer`、`spawn/fork/continuable` 三原语、缓存稳定工具目录 + `CostMeter` + 压缩溯源、内置 `writer-room-team`/`scene-sim` 模板、`MultiAgentCoordinator.run_team` 门面；Writer Room `team` 模式已接入（opt-in `rehearsal_mode=team`）；待 `AgentService` per-session 状态迁移、真实 LLM 端到端验收与旧协调器去重 | `openspec/changes/agent-team-composition/tasks.md` |
