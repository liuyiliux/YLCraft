...
| 数据库迁移收敛 | Alembic 迁移链当前到 `015_add_video_task_kind`；启动和 Agent 请求路径不再隐式改 schema，新增视频/图转 3D/动态状态持久任务均通过显式迁移落库 | `backend/alembic/versions/`、`openspec/changes/database-migration-convergence/tasks.md` |
