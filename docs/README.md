...
| 内容包工作台 | 进行中（12/21）：六种 `package_type` 的契约 schema（`article_package`/`social_carousel`/`shot_list`/`single_media` 为 API-only）、五个平台适配器 + 输出端点（公众号/小红书/短视频/PDF/素材包，按方案的 `output_adapters` 出，带溯源且不复制 items）、条目级重试与 `stale` 语义、`ContentPackagePlanner` 提取（`generate_outline` 变 39 行兼容委托）。剩余：`page_book` 完整实现、Skill/Agent 工具契约、多平台 UI 复用组件、绑定项目流程、Director 路由、测试/smoke/文档收尾 | `openspec/changes/content-package-workspaces/tasks.md` |
