...
| 外部 Agent API | 已有能力发现、素材上传、生图/生视频/3D、任务、事件日志和素材详情接口；外部 Agent 仅通过平台 API 使用页面已配置的连接器，不接触供应商密钥。已落地平台级 `ExternalApiKey` 鉴权（`/api/v1/external-api-keys` 生成/列出/撤销，Bearer 校验 + read/write/generate 作用域 + 每 key 限流），并挂到生图接口；待消耗型操作确认与生产强制策略 | `docs/guides/external-agent-api.md`、`openspec/changes/content-production-orchestration/tasks.md` |
