...
| AI 渐进世界构建（梯子原则） | 进行中：可扩展底座已落地（`world_domain_definitions` 迁移 038、`WorldDomainService`、`GET\|PUT\|DELETE /api/v1/projects/{id}/world-domains`，补齐宗教/语言/文化/生态四域，AI 建议结构需确认才生效）；待实现三档生成动作（`draft_world` / `expand_domain` / `expand_entity`）、项目级提示词模板与 `prompt-preview`、生成与提取的来源隔离 | `openspec/changes/ai-progressive-world-building/` |
