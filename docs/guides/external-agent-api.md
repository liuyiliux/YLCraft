...
| 调用方 | 用哪种凭据 | 用途 | 状态 |
| --- | --- | --- | --- |
| **人类用户** | **登录会话**（服务端会话表 + `HttpOnly` Cookie；备选 Bearer/JWT 见 design 的代价说明） | 浏览器界面 | 已提供：`POST /api/v1/auth/register`、`/login`、`/logout`，`GET /me`；前端通过登录页、会话恢复和受保护业务路由使用 Cookie，不在浏览器存 token |
...
2. **两类凭据不互相替代**：界面会话不能被外部 Agent 复用，`ylk_` Key 当前也不代表某个具体用户。会话用户创建的项目、图片/视频/图转 3D 资产及其任务会写入 `owner_user_id`；外部 Key 仍可通过凭据门槛和既有 scope/quota 校验，但因无用户映射不会伪造 owner。迁移前的历史数据 `owner_user_id` 为空并**仍允许访问**；
3. **开放公网的前置条件**（未完成前不要把服务暴露到公网）：后端登录/会话、登录失败限流、前端登录/会话守卫，以及核心项目/素材/媒体任务的认证与归属过滤已具备；剩余业务写路径和全量消耗型操作仍未完成。公网 HTTPS 部署还必须设 `YLCRAFT_SESSION_COOKIE_SECURE=1`。
