...
状态更新时间：2026-08-31。角色专项已完成两趟小说角色提取、预览确认写入、证据校验、项目增量合并、全局角色复用、正文上下文注入以及真人用户/API 和 AI Agent 双入口验证。
...
| 创作项目闭环 | 部分 | 角色提取、角色库同步、项目回流、正文角色上下文、真人/API 与 Agent 双入口已验证；仍留 1 项真实供应商生图环境验收 |
| 资产中枢（Asset Hub） | 完整 | 图片 / 视频 / 音频 / 文本等统一资产模型、版本、表示、谱系和项目关联已接入主线 |
| Story / Writer Room | 部分 | 大纲、章节、正文、脚本、分镜、叙事上下文、连续性与生产工作台可用；角色设定已注入章节上下文，候选读取已收敛为项目最新与当前章历史分读。工作台前端已于 2026-09-11 完成模块化拆分（`pages/story/index.tsx` 13167 → 10 行：逻辑下沉 9 个 hook、视图拆 11 个组件、类型/样式/工具独立成文件），总览新增「最近活动」；1440/1280/移动三档与 8 个工作区页签已冒烟通过（0 控制台错误），仍留真实供应商生图与深度写作门禁验收 |
| 角色工作区与角色提取 | 完整 | 角色先行创建、小说/正文两趟提取、别名与原文证据、人工合并候选、YLCraft 设定卡、项目世界覆盖、角色库同步和立绘参考链路已接通；真实供应商生图仍受创作项目闭环验收项约束 |
...
产品主链路是「创作项目 ↔ 素材库 ↔ 采集下载 ↔ 小说 ↔ AI 创作」的循环，其他模块不应阻塞这条主线。顶级导航只保留概览、创作项目、素材库和任务中心；角色、3D 预演、创作画布归入“创作工作台”，AI 图片与视频、图生 3D、多平台生图、智能体归入“AI 创作”。

| 入口 | 路由 | 角色 |
|------|------|------|
| 创作项目 | `/story` | 大纲、章节、正文、脚本、分镜、参考与内联生成 |
| 素材库 | `/assets` | 下载 / 生成 / 参考的持久记忆 |
| 下载 | `/download` | 外部文件进入本地资产系统（位于“采集与下载”分组） |
| 小说 | `/novel-bookshelf` | 搜 / 下 / 读 / 换源，作为项目素材源 |
| AI 图片 | `/image-gen` | 由项目提示词生图并回流到项目链接（位于“AI 创作”分组） |
