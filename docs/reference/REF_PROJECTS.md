...
## 观察项：System One 决策模型（Jev / Laya）——记住有这类东西，暂不接入（2026-09-22）

> 记录目的：**以后遇到"高频枚举决策"或"模型老编造枚举值"时，回来翻这一节**。
> 证据等级：官方博客/发布报道 + 媒体转述，**性能与价格均为厂商自述**，未核实；未做过任何接入验证。

### 它是什么

| 项 | 内容 |
| --- | --- |
| **Jev** | TypeSafe AI（创始人 Diogo Almeida，前 OpenAI、参与 RLHF）2026-09-15 发布，自称首个 **System One Model**：**不生成文本**，输入"结构化 state + 类型化问题"，直接返回**带置信度的类型化决策** |
| 定位 | 替代的不是 ChatGPT，而是软件里那些高频、重复、本可用规则/判定完成的事 |
| 自述指标（未核实） | 比传统大模型快约 190 倍、便宜约 440 倍；输入约 $0.042/百万 token、输出免费 |
| 形态 | **闭源云 API**，目前早期访问 |
| **Laya** | 2026-09 出现的**开源** System 1 决策模型家族（非自回归，强化学习构建，公开论文/权重/数据集），自述比 Jev 快约 8 倍、可自部署、无 API 成本 |

### 能不能用它"生成姿势"——不能

它**不是生成器**：输出是"有限枚举里选一个 + 概率"，给不出连续的关节角度（我们要的 23 通道是连续值 + 限位）。所以"让 Jev 生成出拳姿势"这条路不成立；姿势要么引预设、要么由助手/人给角度（见本仓 `poseJoints` 通道）。

### 但它对我们有一处**结构性**价值（记住这一条）

> **类型化输出 = 不可能产出枚举外的值。**

这正是我们在 `previs-agent-scene-composition` 里反复挨打的问题：模型编造 `motion:motionwave`（库里是 `motion:wave`）。Jev 这类模型的输出空间被类型约束住，"编造"在结构上就不可能发生。

**适合替换的环节**（都是"结构化输入 → 有限枚举"）：分镜 `action:"挥手"` → 匹配到哪个动作 slug；景别/运镜类型分类；动作条目分类打标；高频重复的合法性判定。

### 为什么现在不接

1. **我们已有防线，且是确定性的**：动作清单注入消息正文 + 后端枚举校验 + 前端对"前缀叠写"的确定性修复。Jev 给我们的是**更快更省**，不是**解决新问题**；
2. **闭源云 API + 早期访问**：国内可达性、合规、稳定性都要先过一遍；
3. **有更契合的开源替代 Laya**（可自部署、零 API 成本），与我们一贯的取舍一致——**不绑外部服务、离线优先**（与上一节否掉 SMPL-X 那条路是同一条理由）；
4. 现阶段瓶颈不是这类调用的成本，而是**姿势库太薄**——先把库养起来更划算。

### 触发条件（满足任一条就回来评估）

- 枚举决策类调用量大到成本/延迟心疼；
- 编造枚举值靠现有防线拦不住（换模型/换场景后复发）；
- 出现需要低延迟批量分类的场景（如整章几十格的分镜批量打标）。

**顺序**：先试 **Laya**（可自部署、许可干净），再考虑 Jev（闭源云）。

### 待核实（不猜）

- Jev 官方定价、速率限制与"类型化输出"的 API 具体形态（是否支持任意枚举 schema）；输出免费是否长期成立；
- Laya 的**许可证与模型体积**、是否有官方推理服务；
- 两者在国内网络下的可达性；
- 决策模型在"中文分镜语义 → 动作枚举"这类任务上的实际准确率（**必须实测，不能看宣传**）。

***

## 补充调研：视频→动作管线的工程取舍，与迪士尼"生成式关键帧"（2026-09-22）

> 来源：用户提供的两篇公众号文章（非仓库、非论文原文）。
> **第一篇是对上文 B 那条路线的文档级梳理**（同一类开源管线，另一作者整理）：许可红线不变，但补了几条**值得抄的工程取舍**。
> **第二篇是研究发布（不是可下载工具）**，价值在**思路**：它给"AI 生成运动"指了一条与我们既有结构同构的路。

### A. 视频 → Mixamo FK 动画：许可红线不变，但工程取舍值得抄

| 它的做法 | 对我们的意义 |
| --- | --- |
| **定位是"动画前处理管线"，不是一键按钮**；明确未核验为 Unity/Unreal 插件，建议当作"进入 DCC 之前"的候选 | 与我们"不替代 Blender/Unreal/Maya"的边界**完全一致**（上一个 change 的非目标）。印证 |
| **层层可检查的中间产物**：landmarks → 带节拍的 `action_spec` JSON → Blender 骨骼姿态 → QA 与参考对比 | **这正是我们"动作即数据"的加强版**：我们已有"动作条目 = 结构化 JSON"，但**没有"从原始数据到条目的中间产物留痕"**。新动作入库时值得记"[来源 → 中间表示 → 最终条目]"（本期动作是手写的，将来做 BVH 导入时用得上） |
| **FK-only，不把 IK 当默认补救层**：好处是结果可审查，代价是脚底接触/遮挡必须前置到规格层、后置到 QA | 与我们"参数型动作由不变量兜底"同类：**把问题挡在数据层，而不是运行时补救**。我们已有的脚底贴地/膝不反折断言就是这条 |
| **"脚本没报错" ≠ "片段可交付"**：脚底滑动、身体接触、双人穿插、动作边界仍需人看结果 | **正好解释我们的双轨**：数值断言负责"不可能错"的部分，人负责"好不好看"。任何声称"全自动"的动作管线都该被这句质疑 |
| **双人样例的检查点**：两人相对位置、交互时序、是否互相穿插 | 我们刚做的 `punch`/`hit` 定格正需要这三条**人工**判据（数值断言管不了"像不像打上了"） |
| 适用位置里点了**"镜头预演"**与"AI 生成动作的人工复核" | 说明预演阶段确实是这条管线的常见落点——我们的"分镜 → 3D 预演"占了这条位置 |
| **前置条件与红线（与上文一致，再加一条）** | GVHMR + **受门控注册的 SMPL-X**、NVIDIA GPU、Blender 5.1+；**Mixamo 角色需 Adobe 授权，仓库里的展示角色不可当作可再分发资产**——与"Mixamo 禁止把动画当独立资产再分发"是同一道门 |

**结论：仍然不做，但把"中间产物留痕"记进将来做 BVH 导入时的做法（最小切片第 2 步的产物表）。**

### B. 迪士尼 Generative Motion Rig（GMR）：研究，不是产品；但思路对我们有用

**它是什么**：Disney Research Studios（+ ETH Zurich）在 **SIGGRAPH Talks 2026** 的研究，配一个**闭源原型插件**（**下载不了、装不上**）。核心是**生成式关键帧**：动画师给少量稀疏关键姿势，模型生成一整段自然运动；可用噪声采样快速出多个变体；用 scheduled inpainting 编辑/延长动捕剪辑。模型跑在 GPU 服务器上，**Blender 只当前端 UI**。

| 它的点 | 对我们的意义（**先分清层**） |
| --- | --- |
| **它是"角色身体怎么动"，不是"镜头怎么动"** | GMR 补的是**角色的骨骼运动**，对应我们的**动作层**（`previs_motion_assets` / `carrier=params` / `assign_motion`），**不是运镜层**（`cameraMoves.ts` 的机位模板）。两层不要混——相机只有 6 个自由度且规则简单、由我们自己写死；角色 23 通道且有生理约束，才是"需要 AI 补全"的那一层 |
| **"生成式关键帧"= 稀疏关键帧 → 补全整段** | 与我们**动作条目**的形态同构（逐通道关键帧），所以将来真要做"AI 生成一段动作"，正解是**给少量关键姿势让模型补全**，而不是逐帧生成。**但近期价值很低**：我们的动作只有 23 个通道、条目短、还有不变量测试兜底，**手写更可控**（现有 15 条种子动作 + `punch`/`hit` 都是手写） |
| **变体交互**：同一意图采样出多个候选让用户挑 | 贴切的落点是**动作层**（同一动作出幅度/节奏不同的几版）或**方案层**（助手一次给几个方案挑）。**我当时把它说成"运镜给候选"是过头了**——那是我的延伸，不是文章的主张，与 GMR 无关 |
| **模型跑 GPU 服务器、Blender 只是前端** | 与"预演台要轻、要能随时跑"直接冲突；**不引入**。我们的取舍仍是"浏览器内、零外部算力" |
| **论文 ≠ 产品**：工程化/产品化/商业化三道坎；媒体把"研究发布"当"工具上线" | 这条提醒对我们同样适用：**别把研究里的效果当既有能力**。我们自己的文档也一直按这个口径写（未核实的点单列、不猜） |

**结论**：作为**观察项**记录（不可用、需重型算力、闭源）。**它落在动作层，不落在运镜层**；对我们现有的手写动作库**没有近期行动价值**，只提供一条"将来做 AI 生成动作时的正确形态"（稀疏关键帧 → 补全）。**不因此新增任何运镜能力**。
