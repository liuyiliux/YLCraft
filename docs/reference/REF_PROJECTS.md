...
| 它的点 | 对我们的意义 |
| --- | --- |
| **"生成式关键帧"= 稀疏关键帧 → 补全整段** | **与我们的结构同构**：我们的运镜模板就是"给定起点机位与帧区间 → 生成铺满的关键帧"，动作条目是"逐通道关键帧"。将来若要"AI 生成一段运动"，**应当走"稀疏关键帧 → 补全"**，而不是逐帧生成——后者既贵又难审 |
| **变体交互**：同一意图噪声采样出多个候选，让用户挑 | **我们现在缺这一档**：运镜模板只有"强度档位"，没有"再来一版 / 给我三个候选挑"。这是个**便宜且体感强**的小项——候选之间只要在幅度、节奏（如首尾停留）上错开即可 |
...
**结论**：作为**观察项**记录（不可用、需重型算力），但**"稀疏关键帧 → 补全"**与**"变体让用户挑"**两条思路记下来；后者已作为候选小项写进当前 change 的观察项。
