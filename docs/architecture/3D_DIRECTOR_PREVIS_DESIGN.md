...
状态：**Phase 1 与 Phase 2 已落地**（见 `openspec/changes/3d-director-previs/tasks.md`）。Phase 1：`PrevisSceneDocument` 持久化（支持独立场景创建，无需项目）+ revision 并发保护、`/story` 分镜入口与顶级导航入口、可复用 3D 渲染原语、静态节点管理（Asset Hub 模型/可摆姿势的人形占位/几何体/全景背景/图层可见性/重命名/删除/锁定）、相机 CRUD 与导演/活动机位双视角、安全框/九宫格只读叠加、**截图回流**（Asset Hub + `ProjectAssetLink(role=storyboard_reference)` + 分镜参考白名单）。Phase 2：**24fps 时间轴与逐通道关键帧**（旋转 slerp）、节点变换编辑（数值面板 + 3D 手柄）、灯光节点、真实阴影、**复用模型自带动画 clip**（按帧同步）。另完成镜头光学（焦距/画幅/景深）与导出能力评估（见上方 Phase 4）。**待做**：场景级撤销/重做、Agent 导演助手（Phase 3）、批量参考帧导出与服务端合成（Phase 4）。
...
- 每场景只启用 24fps；支持时长、播放头、逐帧、关键帧增删。**（已完成）**
- 支持对象 transform、相机 target/FOV、动画 clip 的基础关键帧。**（已完成）**
- 提供场景级撤销/重做和 revision 冲突提示。**（部分：revision 冲突提示已在 Phase 1 落地；撤销/重做尚未实现——目前只有随场景持久化的操作历史，是审计线索而非撤销栈。这是一处如实标注的缺口。）**
- 先完成参考帧截图；浏览器视频导出作为实验能力，不让它阻塞核心分镜生成工作流。**（参考帧截图已完成；视频导出的评估见 Phase 4。）**
...
- 新增受限的场景操作 Tool schema、锁定校验、revision 校验和确认差异预览。**（已完成）**——落地为**两个工具**而非一个：`previs_preview_operations`（read，只校验+出差异）与 `previs_apply_operations`（write，再校验一次后 CAS 落库）。**拆开是刻意的**：合并成一个就等于让 Agent 自己决定"要不要落库"，确认环节会被跳过。落库**重新校验一次**（预览通过不代表落库时仍通过），`expected_revision` 不匹配时**整批作废**而非挑能用的执行（挑几条会拼出没人预料的中间态）。纯逻辑收在 `services/previs/operations.py`，不碰数据库，可完整单测。**（2026-09-19 扩展，`previs-agent-scene-composition` 任务 3.1–3.5）**：`add_node` 补节点种类白名单与逐类 metadata 校验（原来整包接收，幻觉出的 `kind` 会"落库成功但前端不认"）；新增语义操作 `set_human_proxy` / `assign_motion`，把"设身高、摆姿势、指定动作"从"往 metadata 里塞键值"变成一等操作；动作清单以参数注入（校验器不查库），**清单不可用时拒绝任何动作引用**而不是放过。
- 将已确认操作写入 Agent Run steps 与场景操作历史。**（部分）**——**场景操作历史**已完成：每条已应用操作写入随场景持久化的 `operations`（含类型、目标 ID、可读性摘要）。**Agent Run steps** 未显式写入：工具由 `ToolRegistry.execute_tool` 调用、**拿不到 run_id**，无法自行写 step；目前 trace 走 `AgentService._log_tool_call` 对每次工具调用的自动记录（含完整参数与返回值，**但 result 截断 2000 字符**）。因此工具的返回值刻意保持精简，完整细节放在场景操作历史里。这是约束下的取舍，若要显式写 steps 需先给工具传上下文。

### Phase 4：供应商动作与高级输出

- 结合已验收的腾讯绑骨模型，映射动画 clip 和动作预设。**（部分：复用模型自带动画 clip 已完成；动作预设库未做。）**
- **评估已完成**：浏览器 `MediaRecorder`/WebCodecs 视频导出、FFmpeg 服务端导出和成本边界见 `docs/research/research_report_previs_export_feasibility.md`。结论是**实时录制不作为主路径**（真实负载下播放时 17% 的帧超出 24fps 预算，按墙上时钟打时间戳会把抖动固化进产出），改为**确定性离线导出**：逐帧渲染取图 → 服务端按固定帧率合成；批量帧用 JPEG、单帧截图保持 PNG。全程零 API 额度。
