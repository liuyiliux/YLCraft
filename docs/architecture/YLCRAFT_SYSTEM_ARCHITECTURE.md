...
**生成资产接入（第二期）**：全景背景从"只支持纯色"扩到**从素材库选贴图**（`metadata.textureUrl`）——按扩展名筛可加载的图片而不是按素材分类，分类口径由别处决定，宁可多列也不要"一条都选不到"；渲染侧**自己加载、失败回落纯色**（不用会 suspend 的加载器，避免加载失败把整个画布打掉），**有贴图时材质 color 设白**（否则纯色当染色叠在贴图上会让画面整体偏暗），既有口径不变（内表面球体 + 半径固定、不参与节点尺寸）。模型生成**复用既有 `model3d` 链路**（`/3d/convert`、`/3d/extract-metadata`、`/3d/generate-preview`、`/3d/generate-from-image` 及其轮询、`/3d/supported-formats`），不另造入口；全景贴图的**直接生成**（接生图后端）需额度开关，本期未接。
