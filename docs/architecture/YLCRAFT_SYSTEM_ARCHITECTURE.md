...
**预演台里的对话入口**（2026-09-22，`previs-ai-copilot` 第一期）：预演台右侧内嵌可折叠对话栏，助手角色是**新建的** `previs-assistant`——只持只读三件套（`list_previs_motions` / `get_previs_composition_options` / `generate_previs_draft`）加 `previs_preview_operations`，**刻意不含写工具**：对话只能提方案，落库仍然只有"用户点确认"这一条路。三条口径值得记住：

1. **场景事实必须写进消息正文**，而不是只塞进 `context` 字段——实测后端不把 `context` 呈现给模型，助手于是说"当前会话没有可用的画面场景"、反过来向用户要场景 ID。正文里同时给出**动作标识清单**（原样可用的值，防编造）、**"库里没有的动作不许硬凑标识"**（改用静态姿势 + 站位表达并说明这一版能到哪一步）、以及 **`poseJoints` 通道**（库里没有的姿势自己算关节角度，字段语义与限位一并给出）——不告诉它有这条路，它就只能编一个不存在的名字。
2. **提取出的操作在送校验前做一次确定性修复**：模型最常见的拼造形态是"前缀叠写"（`motion:motionwave`，正确是 `motion:wave`），只做去空白/补去前缀/去叠写这类**确定性**清理，且**库里确有完全同名条目**才替换；修不出来原样交给校验拒掉。**不做模糊猜词**——`sit` 与 `sit-down` 的包含关系会让"修复"修出一个错误的动作。
3. **拒绝原因要翻译成"下一步做什么"**：版本过期 → 判定"这份方案整批作废"并**重新载入最新版本、丢弃幽灵态**（否则用户会拿着基于旧版本的方案反复点确认）；对象被锁定 → 先解锁；动作不存在 → 附上可用动作清单；资产不存在 → 从可摆清单里选。通过与不通过的分界是硬的：**只有校验通过的那个 `proposed_scene` 才会进幽灵态**。

**姿势库的两条扩展与一条老坑**（2026-09-22）：① **预设姿势扩到 8 种**，新增 **`punch`（出拳）/ `hit`（受击后仰）** 这一对——打斗这类库里没有的动作，用两个定型姿势把"意图"摆出来（右臂前伸到脸高度、肘近伸直、左手护脸、躯干转向出拳侧、双腿前后发力；受击方头后仰、躯干后倾到限位、两臂外甩），够分镜预演读站位与构图，**接触级细节不在预演范围内**（连专业动捕方案都要靠碰撞约束才算得清）。② **"我的姿势"预设**（`frontend/src/pages/previs/customPoses.ts`）：把调好的、或助手算出来的姿势**存起来复用**（面板底部起名保存、姿势下拉分组显示、可删、上限 24），存的是**关节角**而不是预设名——预设名要前后端两处枚举同步，而关节角走既有的 `poseJoints` 通道（逐字段校验 + 限位收敛都是现成的），不动任何枚举；存在 localStorage（本机的个人习惯件，不进场景数据），坏数据**逐条跳过**（一条脏数据不该让用户丢掉全部积累）。
> **老坑**：前后端的姿势枚举**必须同步**——曾出现"前端加了 `punch`/`hit`、后端没加"，直接写库（建场景）不报错，而助手一用 `set_human_proxy` 就被以"未知姿势"拒绝。这类不一致**只在真机走工具链时才暴露**，所以新增姿势时两处一起改（代码里已写明）。

**生成资产接入（第二期）**：全景背景从"只支持纯色"扩到**从素材库选贴图**（`metadata.textureUrl`）——按扩展名筛可加载的图片而不是按素材分类，分类口径由别处决定，宁可多列也不要"一条都选不到"；渲染侧**自己加载、失败回落纯色**（不用会 suspend 的加载器，避免加载失败把整个画布打掉），**有贴图时材质 color 设白**（否则纯色当染色叠在贴图上会让画面整体偏暗），既有口径不变（内表面球体 + 半径固定、不参与节点尺寸）。模型生成**不在预演台发起**（2026-09-22 决策）：**预演台的定位是"自动搭建布景"**——把已有的人形占位与姿势/动作、几何体、素材库模型、全景背景、机位与运镜按调度摆进场景，**不承担"造模型"**。模型在图生 3D 工作台生成（复用既有 `model3d` 链路：`/3d/convert`、`/3d/extract-metadata`、`/3d/generate-preview`、`/3d/generate-from-image` 及其轮询、`/3d/supported-formats`），入库后从素材库选进场景。**理由是入参不对**：分镜参考图是"一整个杂乱场景"、不是单体，拿它去图生 3D 得不到可用的独立模型。同理程序化组合体（在基础几何体与外部模型之间加一档参数化组合体）也不做——它必然滑向简易建模器。全景贴图的**直接生成**（接生图后端）需额度开关，本期未接。
