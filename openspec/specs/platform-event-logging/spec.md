# platform-event-logging Specification

## Purpose

为 AI 生成与关键系统事件提供统一的平台级可观测性：把成功/失败都落成结构化事件记录
（含脱敏后的可重放参数），把进程运行日志落盘并可按需查询，在任务中心以三 Tab
（任务 / 事件日志 / 运行日志）统一查看，并允许对失败事件一键重发且保留追溯链。

来源：`openspec/changes/archive/2026-09-11-platform-event-logging`（原 change
`platform-event-logging`，2026-09-11 归档）。

实现口径说明（与实现对齐后的修正）：
- 摘要长度上限为 **20000** 字符（实现常量 `MAX_SUMMARY_LENGTH`，见
  `backend/app/services/platform_log/service.py`）；既保留完整 prompt 与模型原始输出，
  又不会把整本书正文写入事件表。
- 调用类事件的落账已统一收口到 `AIService` 三个入口；端点不再手写同义事件。
...
- **WHEN** 请求/响应摘要超过约定长度
- **THEN** 系统最多保存截断后的摘要（当前上限 20000 字符）
