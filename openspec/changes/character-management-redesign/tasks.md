...
- [x] 2.3 Bible 分区组件重构为紧凑网格：原计划复用的 `CharacterBibleQuickPanels`/`CharacterBibleDetailedPanels` 已随 2.2D 删除，改为新增 `FieldRenderer` 按值类型分流——视觉档案 → `VisualProfilePanel` 分组卡片、对象 → `KeyValueGrid` 两列键值网格（`96px / 1fr`，窄屏降为单列）、数组 → 标签云、标量 → 文本段落；Bible 区字段全部经此渲染，对象型字段不再展平成 `key：value` 文字墙
