...
- [x] 27. Update API surface, architecture, creative workflow guide and database migration docs.
  - _Requirement: 1, 3, 8_
- [x] 28. Validate with human UI and Agent API E2E flows using temporary local fixtures; never use real user/remote novel data for tests.
  - _Requirement: 5, 8, 9_
  - _2026-09-12 实测（fixture 为现场杜撰的 3 章合成小说「灰港纪事」——灰港/雾鲸/殷砂/烛台议会等均非真实作品，未读取任何真实用户或远端小说数据）：_
    - _Agent/HTTP 侧（真实 LLM，非 mock）：`POST /novel-sources/import-txt` → 快照 `completed`；3 章 / 3 文本块；`/plan` 逐域检测出 9 域；`/extract` 运行 `status=success`。_
    - _需求 5（复核闸门）：13 条候选、首条含证据锚点；接受 1 条候选**耗时 0.07s**（远低于一次 LLM 往返），证明复核未二次调用模型；`apply` 后仅落地已接受的 1 个角色（`world-knowledge.counts.characters=1`），其余 12 条仍 `pending`——"显式接受才成正典"成立。_
    - _需求 9（可恢复）：运行对象暴露 `domains`（逐域 `detection` / `reason` / `signals` / `estimated_cost` / `run_state` / `items`）、`failures`、`checkpoint`（`last_chunk_ordinal`）、`trace`、`diagnostics`，满足"逐域进度可检视 + 失败阶段可重试"的结构要求。_
    - _需求 8（人机对等）：Agent 侧 `list_world_extraction_candidates` / `decide_world_extraction_candidates` / `apply_world_extraction_run` / `reconcile_world_extraction_run` 与 HTTP 端点**共用 `WorldExtractionService`**（`list_candidates` / `decide_candidates` / `apply_run` / `reconcile_run`），对等性由同一服务层保证。_
    - _human UI（真实浏览器，非源码 grep）：用 `patchright` 驱动真实 Chromium 访问 `/novel-world`——页面正常渲染；**上一步 E2E 造的快照「烟雾验收-灰港纪事（3 章 · 完本）」真实出现在 UI 列表中**（导入→UI 闭环）；点中该快照后在 UI 内点击「AI 判断模块」，**12 秒**内经真实 LLM 返回逐域判定结果（角色 / 地点 / 势力 / 世界规则 / 历史事件 / 物种 / 剧情时间线）；`/world-map` 渲染 1354 字符含「世界地图 / 据点 / 路线」；全程**零 >=400 响应**，截图见测试产物。_
    - _说明：`npm run smoke:pages` 只是 `readFileSync` + `includes()` 的**源码文本扫描**，既不挂载页面也不启动浏览器，无法代表运行时表现——本次因此改用真实浏览器复验（它通过不等于页面能跑）。_
    - _未覆盖：未在浏览器内把提取→审阅→写入的整条向导走完（会实际消耗 LLM 配额），也**未人为制造域失败**实测 partial 重试路径；此两点由后端 E2E 与单元测试覆盖。_
...
- [x] 36. 项目视觉基准与画风预设（契约 R7，ui-context 反模式第 5 条）。
  - _Done: 新增 `services/creative_project/visual_baseline.py`：视觉基准落地为 `project_asset_links` 中 `role="visual_baseline"` 的一条关联（不新增表、引用素材库节点），一个项目只保留一张（重设即替换），端点 `GET/PUT/DELETE /api/v1/creative-projects/{project_id}/visual-baseline`。生图链路（`world_map_visual.generate_map_visual`）自动注入项目基准到 `reference_asset_ids`（显式传的在前、基准追加在后、去重），页面与 Agent 均自动受益，未设置不阻塞生图。测试 2 例：基准替换语义与 role 隔离、生图自动注入顺序。既有 83 例通过。文档：架构文档补视觉基准段。_
- [x] 35. 生图参考图改传素材库 ID（P1：原为 base64/URL 直传）。
  - _Done: 新增 `services/asset_hub/reference_resolver.py`，把 images.py 里已有的素材库 ID → 图片路径解析（含集合/角色根节点的子节点下钻、`max_refs`、去重合并）抽为共用模块，images.py 改为复用并删掉重复实现与两个失效 import。地图生图 `generate_map_visual` 与端点、Agent 工具同步新增 `reference_asset_ids`，解析结果与兜底 URL/base64 合并去重后送模型，并把真实数量写入血缘参数 `reference_images_count`。前端：VisualDrawer 中已入素材库的历史成图按 `node_id` 勾选（标注「素材库」），未入库的仍按 URL 兜底。新增测试 1 例（素材库 ID 被解析进生图请求），既有 image 相关 37 例与地图 13 例不受影响。顺带修掉既有缺陷：novel_sources.py 后台任务里未导入的 `SessionLocal`（一调用即 NameError）。文档：agent-center 与架构文档同步。_
- [x] 34. 世界地图 Agent 工具组（P1：AI 优化提示词等端点此前无 Agent 入口）。
  - _Done: 新增 `services/agent/tools/world_map_tools.py`，注册 12 个工具（8 read / 4 write），与真人工作台共用 service 层而非各自实现：`list/get/create/save(CAS)/render_svg/export_points/resolve_entities/build_prompt/optimize_prompt/generate_visual/list_revisions/rollback`。视觉编排先下沉到 `world_map_visual.py`（提示词解析、LLM 润色、成图与入资产中枢、本地 URL 解析），两个端点改为调用它，错误处理映射回原状态码、行为不变。回滚在工具层先校验 `expected_revision` 再调 service（service.rollback 内部以当前版本做 CAS）。新增 `tests/test_world_map_agent_tools.py`（6 例：注册与风险分级、CAS 读写与冲突、游离标记、优化不落库、成图只追加引用、版本 append-only 与回滚产生新版本），地区既有 13 例地图测试仍通过。文档：`docs/agent/agent-center.md` 新增「世界地图工具」小节。_
