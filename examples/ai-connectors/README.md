...
| `chybenzun-gpt-image-2-edit.json` | OpenAI | Image to image | OpenAI GPT Image-2 edits protocol example using custom HTTP multipart; replace the placeholder Base URL with a trusted provider. |
...
| `triposr-image-to-3d.json` | TripoSR | Image to 3D | Upload -> submit -> poll preset matching the legacy TripoSR path. The migration command writes the same connector shape and never prints the API key. |
