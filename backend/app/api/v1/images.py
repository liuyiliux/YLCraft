...
def _reference_image_summary(raw: str) -> str:
    """参考图入参摘要：http URL 原样、base64 data URL 记类型与长度，避免落库超长。"""
    if not raw:
        return ""
...
def _image_log_request(req) -> dict:
    """事件日志「调用详情」：含业务字段与参考图来源摘要，不含图片二进制。"""
    ref_imgs = [
        _reference_image_summary(x)
        for x in (req.reference_images or [])[:5]
    ]
    return {
...
async def _try_create_generated_image_asset_hub(*, context: str, **kwargs) -> str:
    """Persist a generated image without leaking a failed DB transaction upstream."""
    from app.db.database import get_async_session

    try:
        async with get_async_session() as session:
            async with session.begin_nested():
                return await _create_generated_image_asset_hub(session, **kwargs)
