...
"""
    ...
    async def generate_content_package(
        ...
        *,
        ...
        panel_instruction = (
            "这是漫画/绘本：**一条 = 一页 = 一张图**，不要把一格做成一条。\n"
            "\n"
            "**为什么是一页一张图**（实测，不是推测）：整页分镜页由模型一次画完，格数、格间白色\n"
            "间隔与黑色描边框、留白气泡都能正确排出。按格出图会多花约 5 倍调用（22 页 × 5 格\n"
            "≈ 110 次 vs 22 次），而且后期还得自己拼版。\n"
            "\n"
            "每条（= 每页）要给出：\n"
            "- `page_index`：页码，从 1 起。\n"
            "- `panels`：该页画格的**数组**（不是格数）。**每页 3-6 格**——"
            "「整页单格」只留给冲击或停顿（跨页效果），不要每页都只有一两格，"
            "那会把节奏拖平（实测漏写这条时模型只给 2 格/页）。每格含：\n"
            "  - `panel_index`：该页第几格，从 1 起。\n"
            "  - `position`：**版面位置**，如「上方横长格」「中部左」「中部右」「中下」「下方横长格」。\n"
            "    **必须写**——不写位置模型会自由发挥版面（实测最常见的翻车点之一）。\n"
            "  - `shot`：景别与镜头（大远景/中景/特写/主观视角…）。逐格变化，不要每格同一景别。\n"
            "  - `description`：该格画面，**一句话**（景别 + 主体 + 动作）。一格里塞两个动作会糊。\n"
            '  - `dialogue` / `sfx`：对白与音效原文。形状必须是：\n'
            '    "panels": [{"panel_index": 1, "position": "上方横长格", "shot": "大远景", '
            '"description": "……", "dialogue": "……", "sfx": "……"}, {…}]\n'
            '    **要真的写出来，不要留空**——只给形状不给要求时，模型会把这两项一律填成空串，\n'
            '    结果整页没有一个字（实测：15 页 30 格全部空着，只好再单独跑一遍补对白）。\n'
            '    规则：\n'
            '    · `dialogue` 是**气泡里的话**，中文 4~18 字。气泡容量有限，长了会溢出。\n'
            '    · 符合人物：不知道该说什么时，宁可让这一格空着，也不要写「……」充数。\n'
            '    · **纯氛围格不要一律留空**：大远景、空镜、环境细节这类没有人物台词的格，\n'
            '      用一句简短的**旁白**交代画面或气氛（旁白同样是台词，也填在 `dialogue` 里）。\n'
            '      实测若只写"没台词就留空"，模型会让一页里多数格都空着，整页没几个字、\n'
            '      看起来像没做完——而漫画里空镜配旁白是常规做法。\n'
            '    · `sfx` 是**拟声字**，即"这个声音听起来像什么字"：咚、哐、吱呀、嘶——、窸窣、\n'
            '      咔啦、隆隆。**不是状态词或形容词**——「颤抖」「恐怖」「紧张」不是声音，\n'
            '      实测模型会这样填错。没有明显音效就留空字符串。\n'
            "- `image_prompt`：**整页**提示词，按下面结构写——\n"
            "  ① 开头声明：「一整页漫画分镜页，纵向排版，共 N 格，格子之间有清晰白色间隔与黑色描边框，\n"
            "     页面内留有对白气泡的位置但**气泡内绝对不要写任何文字**」。\n"
            "     最后这句必须原样写：模型渲染中文很差，不禁止就一定出乱码。\n"
            "  ② 逐格列出：「第k格（位置）：景别。场景。人物。动作/表情。」每格一句话。\n"
            "  ③ 结尾**逐页原样重复**风格块与负面块（省了画风就会漂）：风格块取制作圣经的\n"
            "     image_style_prompt，负面块取制作圣经的 negative_prompt。\n"
            "- **对白不要写进 image_prompt**：一律后期贴字（模型写中文会乱码）。\n"
            "\n"
            "**页数是「展开」出来的，不是「概括」出来的。**\n"
            "常见错误是把整段故事压成十来页——那是插画配图，不是漫画。页数来自把每一拍拆成镜头：\n"
            "一个动作、一次回头、一次视线转移、一处环境细节、一次呼吸的停顿，都可以各自成格，\n"
            "若干格再组成一页。例如「主角回头看见村子在远处」：漫画里通常是 4 格（①还清晰\n"
            "②被树挡一半 ③只剩几盏灯 ④几乎全黑）——恐惧是**累积**出来的，压成一格就没有了。\n"
            "按这个尺度：一千字左右的故事，在连载漫画里通常展开成 **15-25 页**。\n"
            "这个数只用来校准尺度，不是要凑的目标——仍按内容决定，但宁细勿粗。\n"
            if is_panel_based
            else ""
        )
        item_fields = (
            "index、title、text、image_prompt、video_prompt、page_index、panels"
            if is_panel_based
