...
- 公开视频：直接走 yt-dlp（传 cookie 文件）
- 无水印视频：尝试桌面端 API（`services/video/parser_douyin.py`）
- Cookie 传递：使用 cookie 文件路径（Netscape 格式）
"""
...
logger = logging.getLogger("ylcraft.download.douyin")
...
class DouyinDownloader(BaseDownloader):
    ...
    def _get_cookie_file(self) -> Optional[str]:
        ...
        try:
            ...
            if path and os.path.exists(path):
                ...
                logger.info(f"[DouyinDownloader] cookie 文件: {path}")
                return self._cookie_file_path
        except Exception as e:
            logger.warning(f"[DouyinDownloader] 获取 cookie 文件失败: {e}")
        return None
    ...
    async def parse(self, url: str) -> Optional[VideoInfo]:
        ...
        策略：
        1. 先尝试桌面端 API（`parser_douyin.py`）
        2. 失败则返回 None，让调用方降级到 yt-dlp
        """
        try:
            from app.services.video.parser_douyin import parse_douyin_desktop
            result = await parse_douyin_desktop(url)
    ...
    async def download(
        ...
        logger.info(f"[DouyinDownloader] 开始下载 | url={url[:80]} | cookie_file={cookie_file or 'None'}")
        ...
        def _download():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not os.path.exists(output_path):
                    candidates = list(savedir.glob(f"douyin_{safe_title}_*"))
                    if candidates:
    ...
    def _build_ydl_opts(
        ...
        if cookie_file:
            ...
            logger.info(f"[DouyinDownloader] yt-dlp cookiefile={cookie_file}")
    ...
    async def get_qualities(self, url: str) -> list:
        ...
        抖音通过 yt-dlp 下载时，清晰度由 format 参数控制。
        这里返回常见选项供前端展示。
        """
        return [
