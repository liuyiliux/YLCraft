...
class GeminiImageBackend:
    ...
    async def generate(self, req: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        try:
            ...
            return ImageGenerationResult(
                success=False, error=error_msg, provider=self._name, model=model, latency_ms=latency_ms
            )
        ...
        if req.reference_images:
            for ref_path in req.reference_images:
                try:
                    ...
                    logger.info(f"[Gemini-Image] 添加参考图: {ref_path} ({len(img_data)} bytes, {mime_type})")
                except Exception as e:
                    logger.warning(f"[Gemini-Image] 读取参考图失败，跳过: {ref_path}, 错误: {e}")
        return contents
    ...
    def _extract_and_save_images(self, response) -> list[str]:
        ...
        for idx, part in enumerate(candidate.content.parts):
            if part.inline_data and part.inline_data.data:
                ...
                local_path = self._save_image_bytes(img_bytes=img_bytes, mime_type=mime_type, index=idx)
                if local_path:
                    saved_paths.append(local_path)
        return saved_paths

    def _save_image_bytes(self, img_bytes: bytes, mime_type: str, index: int = 0) -> Optional[str]:
        try:
