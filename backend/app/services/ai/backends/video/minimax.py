...
class MinimaxVideoBackend(BaseVideoBackend):
    ...
    def __init__(self, api_key: str, api_base: str = "https://api.minimax.chat/v1", model: str | None = None):
        super().__init__(
            name="minimax-video",
    ...
    def video_capabilities(self) -> VideoCapabilities:
        return VideoCapabilities(first_frame=True, last_frame=False, reference_images=False, max_duration=10)
    ...
    async def _generate(self, req: VideoGenerationRequest) -> VideoGenerationResult:
        ...
        return VideoGenerationResult(success=True, task_id=task_id, status="pending", duration_seconds=req.duration)
    ...
    async def _poll(self, task_id: str) -> VideoGenerationResult:
        ...
        status_map = {
            "pending": "pending", "processing": "processing", "running": "processing",
            "succeeded": "done", "completed": "done", "failed": "error", "expired": "error",
        }
        mapped_status = status_map.get(status, status)

        if mapped_status == "error":
            error_msg = data.get("error") or data.get("message") or "Unknown error"
            return VideoGenerationResult(success=False, task_id=task_id, status="error", error=f"MiniMax 视频生成失败: {error_msg}")
        ...
        if mapped_status == "done":
            ...
            return VideoGenerationResult(
                success=True, task_id=task_id, status="done", url=video_url,
                video_path=output_path, duration_seconds=data.get("duration", 5), seed=data.get("seed"),
            )

        progress = data.get("progress", 0)
        return VideoGenerationResult(success=True, task_id=task_id, status="processing", progress=progress, progress_message=f"视频生成中... {progress}%")
